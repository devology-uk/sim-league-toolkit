...
interface PenaltiesPanelProps<TPenalty extends PenaltyItem> {
    penalties: TPenalty[];
    isLoading: boolean;
    onAdd: (data: PenaltyFormData) => Promise<unknown>;
    onDelete: (id: number) => Promise<unknown>;
}

export const PenaltiesPanel = <TPenalty extends PenaltyItem, >({penalties, isLoading, onAdd, onDelete}: PenaltiesPanelProps<TPenalty>) => {
    ...
    const handleAdd = async () => {
        ...
        await onAdd({reason: reason.trim(), penaltySeconds});
        setReason('');
            ...
            {penalties.map((penalty) => (
                <div key={penalty.id} className='flex align-items-center gap-2'>
                    ...
                    <Button icon='pi pi-trash' rounded text severity='danger'
                            onClick={() => onDelete(penalty.id as number)}/>
                </div>
            ))}

...
export const ScoreList = ({scoringSetId}: ScoreListProps) => {
    ...
    const onAdd = () => {
        ...
        if (scoringSetScores && scoringSetScores.length > 0) {
            let lastPosition = 0;
            let lastPoints = 999;

            scoringSetScores.forEach(sc => {
    ...
    const onConfirmDelete = () => {
        setShowDeleteConfirmation(false);
        deleteScoringSetScore(selectedItem.id)
            .then(() => {
                setSelectedItem(null);
            });
    };
    ...
    const onSave = async () => {
        ...
        const formData: ScoringSetScoreFormData = {
            points: points,
            position: position,
        };
        ...
        <>
            ...
            {!isAdding && !isEditing && (
                <Panel headerTemplate={headerTemplate}>
                    <DataTable value={scoringSetScores} size='small'

...
export const StandaloneEventEditor = ({onSaved, onCancelled, standaloneEvent}: StandaloneEventEditorProps) => {
    ...
    useEffect(() => {
        ...
        setName(standaloneEvent.name);
        setDescription(standaloneEvent.description);
        setEventDate(new Date(standaloneEvent.eventDate));
        if (standaloneEvent.startTime) {
            const [hours, minutes] = standaloneEvent.startTime.split(':').map(Number);
            const t = new Date();
        ...
        setIsActive(standaloneEvent.isActive);
        setMaxEntrants(standaloneEvent.maxEntrants);
        setScoringSetId(standaloneEvent.scoringSetId ?? 0);
        setRuleSetId(standaloneEvent.ruleSetId ?? 0);
    }, [standaloneEvent]);

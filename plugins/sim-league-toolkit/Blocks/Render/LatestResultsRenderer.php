<?php
  ...
  class LatestResultsRenderer implements BlockRenderer {
    public function render(array $attributes, string $content, WP_Block $block): string {
      $limit = max(1, (int)($attributes['limit'] ?? 10));

      $results = array_merge(
        ResultSummary::listRecentChampionship($limit),
        ResultSummary::listRecentStandalone($limit)
      );

      if (empty($results)) {
        $emptyAttributes = get_block_wrapper_attributes(['class' => 'sltk-result-list-empty']);

        return "<p {$emptyAttributes}>" . esc_html__('No results yet.', 'sim-league-toolkit') . '</p>';
      }
      ...
      return "<table {$tableAttributes}>"
        . '<thead><tr>'
        . '<th>' . esc_html__('Driver', 'sim-league-toolkit') . '</th>'
    ...
    private function resultRow(ResultSummary $result): string {
      ...
      return '<tr>'
        . '<td>' . esc_html($result->memberName) . '</td>'
